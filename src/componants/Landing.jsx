...
const Landing = ({scroll}) => {
  console.log(scroll)
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{duration:1}} id="Landing" className={`" bg-landing relative top-0 w-full h-full md:w-full md:h-full bg-cover transition-all duration-[1s]  text-white mb-[80px]"`}>
      <div className="w-full h-[110%] md:w-full md:h-full absolute bg-black opacity-[0.9] sm:opacity-[0.5] z-1" />
      <motion.div initial={{x:-400}} animate={{x:0}} transition={{duration:2,delay:0.5 ,type:"spring"}} className="relative z-[5] top-[30%] px-[5%] capitalize font-light   md:text-[50px] tracking-[2px] ">
        <p>ahmed </p>
      ...
      <motion.div initial={{scale:0}} animate={{scale:1}} transition={{duration:2,delay:0.5 ,type:"spring"}} className="relative flex flex-row top-[270px] md:top-[250px] left-[5%] w-full max-w-max gap-[20px] ">
      ...
      <motion.div  initial={{x:400 ,opacity:0}} animate={{x:0,opacity:1}} transition={{duration:1,type:"spring"}}   layout  className="flex mx-auto z-30 max-w-max relative top-[52%] md:top-[45%] gap-5 transition-all duration-300 hover:bg-white bg-black px-5 md:px-7 py-2 rounded-md md:rounded-2xl">
